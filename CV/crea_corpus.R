...
setwd(ifelse(substr(getwd(),1,1)=="E","E:/Dropbox/_Data/RRHH/app","C:/Users/rrive/Dropbox/_Data/RRHH/app"))


DIR_DATA="../data/"
DIR_FILES="../data/"

ANTIWORD=ifelse(substr(getwd(),1,1)=="E","E:/antiword/antiword","C:/antiword/antiword")
...
data_rrhh_files=read.csv(paste(DIR_DATA,"Anonymous_files.csv",sep=""),sep=";",header=T,encoding = "utf-8")

DIR_FILES="E:/laliga/"

#C:\Users\rrive\Dropbox\_Data\RRHH\app


table(data_rrhh_files$Extension)

if (F)
{
  pdf_files=DirSource(DIR_FILES,pattern="*.pdf")$filelist
  word_files=DirSource(DIR_FILES,pattern="*.docx")$filelist
  old_word_files=DirSource(DIR_FILES,pattern="*.doc")$filelist
  rtf_files=DirSource(DIR_FILES,pattern="*.rtf")$filelist
  txt_files=DirSource(DIR_FILES,pattern="*.txt")$filelist
  
  all_files=c(pdf_files,word_files)
  
  all_files=DirSource(DIR_FILES)$filelist
}

all_files=paste(DIR_FILES,data_rrhh_files$File,sep="")


file_RRHH=as.data.frame(t(sapply(all_files,function(x) status_doc_RRHH(x))),stringsAsFactors = F)
row.names(file_RRHH)=NULL  

table(file_RRHH$status)

#Aquí es donde obtengo los textos
txt_RRHH=apply(file_RRHH,1,FUN=function(x) procesa_RRHH(x))

if (F)
{
  file_RRHH$fichero_interno=paste(seq(1:nrow(file_RRHH)),gsub("^.*\\.",".",file_RRHH$fichero),sep="")
}

file_RRHH$fichero_interno=data_rrhh_files$File

file_RRHH$n=seq(1:nrow(file_RRHH))


file_RRHH$leng=unlist(lapply(txt_RRHH,function(x) x[[2]]))
...
apellidos=read.csv("apellidos.csv",header=T,encoding = "utf-8",stringsAsFactors = F)
...
src=VectorSource(sapply(txt_RRHH,function(x) paste(x[[1]],collapse="\n")))



corpus <- VCorpus(src,readerControl=list(language=file_RRHH$leng)) 
...
my_dtm <- DocumentTermMatrix(VCorpus(VectorSource(corpus_anal)),
...
file_RRHH$nword_corpus=rowSums(my_dtm_df)

#write.table(cbind(file_RRHH,my_dtm_df),file="corpus.csv",sep=";",row.names=F)

#save(file_RRHH,file="file_RRHH.RData")
#save(my_dtm_df,file="my_dtm_df.RData")
#save(my_dtm,file="my_dtm.RData")

if (F)
{
  term_tfidf <-tapply(my_dtm$v/rowSums(as.matrix(my_dtm))[my_dtm$i], my_dtm$j, mean) * 
    log2(nDocs(my_dtm)/colSums(as.matrix(my_dtm) > 0))
}

load(file="file_RRHH.RData")
load(file="my_dtm_df.RData")
load(file="my_dtm.RData")
